...
& in menu zetten
Analoge klok maken
Digitale tijd groot printen
Ip-adres na 1 minuut weg
Store serial-print per dag op SD per maand
...
void setup() 
{
 ...
 // pinMode(TFT_LED,    OUTPUT);                                                              // ILI9341 backligh. Using pin mode result in minimal but noticable display flickering 
 
//  adcAttachPin(PhotoCellPin);        // This code can be used to increase the resolution of the analog pin
...
void EverySecondCheck(void)
{
 ...
 if (msLeap >999)                                                                             // Every second enter the loop
  { 
//  Serial.print(millis()); Serial.print("T "); 
   GetTijd(0);   
//   Serial.print(millis()); Serial.print("M "); 
   // Update timeinfo.tm_sec, timeinfo.tm_min, timeinfo.tm_hour, timeinfo.tm_mday, timeinfo.tm_mon, timeinfo.tm_year
   if (timeinfo.tm_min != lastminute) EveryMinuteUpdate();                                    // Enter the every minute routine after one minute
//Serial.print(millis());Serial.print("D "); 
   if (Mem.DisplayChoice == DIGITAL)   PrintTimeInScreen();                                   // Show the digital time in the screen
//Serial.print(millis());Serial.print("A "); 
   if (Mem.DisplayChoice == ANALOOG) DrawAnalogeClockHands();
//Serial.print(millis());Serial.print("L "); 
...
{ 
 if (Mem.DisplayChoice == DIGITAL) return; // { PrintTimeInScreen(); return;}                                //PrintDigital) 
 if (Mem.DisplayChoice == ANALOOG) return; // { DrawAnalogeClockHands(); return;}                                //PrintAnalog)  
