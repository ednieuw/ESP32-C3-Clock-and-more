 ...
 if (msLeap >999)                                                                             // Every second enter the loop
  { 
   ...
   if (msLeap%500==0  && Mem.StatusLEDOnOff)                                                  // Do not update the LED unnecesary often.It takes a lot of msecs per update
       SetStatusLED(0,0,12,12*WIFIConnected,12*deviceConnected);  
...
//        LDRread,LDRavgread,MinPhotocell,MaxPhotocell,OutPhotocell,(int)(OutPhotocell/2.55),Loopcounter);    
   sprintf(sptext,"LDR:%3d (%3d-%3d) %2d%% %5ld l/s ",
...
{ 
 if (Mem.DisplayChoice == DIGITAL) return;                                                    // Is done every second
 if (Mem.DisplayChoice == ANALOOG) return;                                                    // Is done every second
...
void PrintTimeInScreen(void) 
{
 ...
 GetTijd(0); 
Serial.println("Ben in PrintTimeInScreen");
