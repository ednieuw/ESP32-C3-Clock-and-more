...
--------------------------------------  
 ...
 Changes V029: Stable version. 
 Changes V030: Added ESP32-C3-supermini
               For ESP32 with RGBW SK6812 Led strip see ESP32Arduino_WordClockV0nn
 Changes V031: ESP32-V3.0.0 
 Changes V032: Corrected SetRTCtime() struct timeval now = { .tv_sec = t , .tv_usec = t}; --> struct timeval now = { .tv_sec = t};
 ...
 Changed in Setup()
            ...
            --->
          ledcAttach(TFT_LED, 5000, 8);  //ledPin, freq, resolution, ledChannel);                     // ILI9341 backlight on ledchannel 0 and GPIO pin TFT_LED
...
void setup() 
{
 ...
 //ledcSetup(0, 5000, 8);        //ledChannel, freq, resolution);                               // configure LED PWM functionalitites 5000 Hz, 8 bits
 //ledcAttachChannel(TFT_LED, 5000, 8, 0);  //ledPin, freq, resolution, ledChannel);                                           // ILI9341 backlight on ledchannel 0 and GPIO pin TFT_LED
ledcAttach(TFT_LED, 5000, 8);  //ledPin, freq, resolution);                                           // ILI9341 backlight on ledchannel 0 and GPIO pin TFT_LED
...
//--------------------------------------------
void Displayprint(const char *tekst)  
{ 
               #ifdef ILI9341 
  tft.print(tekst); 
                #endif // ILI9341
  }                                 // Print to an attached display
void Displayprintln(const char *tekst)
{
               #ifdef ILI9341   
  tft.println(tekst);
               #endif // ILI9341
